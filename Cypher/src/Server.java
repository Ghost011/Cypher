...
public class Server implements Runnable {
	...
	static List<ServerClient> clients = new ArrayList<ServerClient>();
	...
	private void sendStatus() {
		...
		for (int i = 0; i < clients.size() - 1; i++) {
			onlin += clients.get(i).name + "/n/";
		}
		onlin += clients.get(clients.size() - 1).name + "/e/";
		sendToAll(onlin);
	}
	...
	static void disconnect(int id, boolean status) {
		ServerClient c = null;
